...
class Block:
    ...
    def mine_block(self, should_stop=None, max_attempts=None, on_progress=None,
                   miner=None, deterministic=False):
        ...
        # ⚠️ `deterministic` ЗАПРЕЩАЕТ внешние движки, и это не придирка.
        # Нативный майнер и GPU перебирают nonce ПАРАЛЛЕЛЬНО и отдают тот, что
        # нашёлся первым, — результат зависит от того, какой поток успел. Для
        # обычного блока это безразлично (годится любой валидный nonce), но
        # генезис обязан получиться ОДИНАКОВЫМ у всех узлов без всякой связи
        # между ними: его хеш и есть отпечаток сети (`p2p.network_id`).
        # Поймано так: с собранным майнером два узла на одной машине получали
        # РАЗНЫЙ генезис (nonce 28 и 69), считали друг друга чужой сетью и
        # отказывались соединяться — 17 тестов краснело именно из-за этого.
        if max_attempts is None and not deterministic:
            engine = miner
            if engine is None:
...
class Blockchain:
    ...
    def create_genesis_block(self):
        ...
        ⚠️ `deterministic=True` здесь ОБЯЗАТЕЛЕН, и обещание в первой строке
        держится только благодаря ему. Внешние майнеры (C++, GPU) перебирают
        nonce параллельно и возвращают тот, что нашёлся первым, — на одной и
        той же машине дважды подряд получались nonce 28 и 69. А хеш генезиса
        это отпечаток сети: разошёлся он — и два узла считают друг друга чужой
        сетью, не соединяются и не синхронизируются.

        Метка времени у генезиса тоже нулевая и тоже намеренно: возьми мы
        текущее время, у каждого узла была бы своя цепочка.
        """
        genesis = Block(0, "0" * 128, "B-hydra Genesis Block",
                        timestamp=GENESIS_TIMESTAMP, target=self.genesis_target)
        genesis.mine_block(deterministic=True)
        return genesis
