...
Эталонная реализация блокчейна согласно белой книге B-hydra:
  ...
  * Сложность                 : ретаргетинг по времени, как в Bitcoin —
                                цель пересчитывается так, чтобы средний темп
                                возвращался к целевому времени блока (больше
                                майнеров → блоки идут быстрее цели → следующий
                                пересчёт поднимает сложность);
...
# --- Ретаргетинг сложности по времени (как в Bitcoin) ------------------------
# Каждые RETARGET_INTERVAL блоков цель пересчитывается так, чтобы средний темп
# возвращался к TARGET_BLOCK_TIME: если последнее окно шло быстрее цели — блок
# усложняется (target ↓), медленнее — упрощается (target ↑). За один пересчёт
# цель меняется не более чем в MAX_ADJUST_FACTOR раз (как и у Bitcoin — защита
# от резких скачков). Bitcoin использует окно 2016 блоков; B-hydra — меньшую
# сеть, поэтому окно короче, но механика идентична. Важное следствие: больше
# майнеров → блоки находят быстрее цели → следующий пересчёт поднимает
# сложность. Так «больше майнеров — труднее» получается само, как в Bitcoin.
RETARGET_INTERVAL = 100        # длина окна пересчёта (у Bitcoin — 2016 блоков)
MAX_ADJUST_FACTOR = 4          # цель меняется не более чем в 4 раза за пересчёт
...
class Blockchain:
    ...
    def __init__(self, difficulty=DEFAULT_DIFFICULTY,
        ...
        # Окно ретаргетинга — параметр сети (одинаков для всех узлов). По
        # умолчанию RETARGET_INTERVAL; меньшие значения удобны в тестах.
    ...
    def expected_target(self, height):
        ...
        Между пересчётами цель та же, что у предыдущего блока. На границе окна
        (каждые retarget_interval блоков) цель пересчитывается по фактическому
        времени последнего окна относительно ожидаемого
        (retarget_interval × TARGET_BLOCK_TIME): шло быстрее цели → target
        уменьшается (труднее), медленнее → увеличивается (проще). Изменение за
        один пересчёт ограничено множителем MAX_ADJUST_FACTOR, а сама цель — не
        легче генезиса (база сети) и не нулевая. Целочисленная арифметика —
        чтобы результат был побитово одинаков на всех платформах.
        """
        if height == 0:
            return self.genesis_target
        interval = self.retarget_interval
        prev_target = self.chain[height - 1].target
        if height % interval != 0:
            return prev_target                            # внутри окна — без изменений

        # Граница окна: пересчёт по времени последних `interval` блоков.
        first = self.chain[height - interval]
        last = self.chain[height - 1]
        actual = int(last.timestamp) - int(first.timestamp)
        expected = int(interval * TARGET_BLOCK_TIME)
        lo, hi = expected // MAX_ADJUST_FACTOR, expected * MAX_ADJUST_FACTOR
        actual = max(lo, min(hi, actual))                 # ограничение скачка ×4
        actual = max(1, actual)
