...
# Награда за блок: строго 50 BHY, и каждые HALVING_YEARS лет она делится пополам
# — как халвинг Bitcoin (~раз в 4 года). Время блока подобрано так, чтобы
# HALVING_INTERVAL блоков добывались ровно за HALVING_YEARS лет.
GENESIS_YEAR = 2026
HALVING_YEARS = 4
...
# Время блока: HALVING_INTERVAL блоков = HALVING_YEARS лет → халвинг раз в 4 года.
BLOCK_TIME_SECONDS = HALVING_YEARS * SECONDS_PER_YEAR / HALVING_INTERVAL
...
class Blockchain:
    ...
    def block_reward(self, height):
        ...
        Строго 50 BHY, и каждые HALVING_INTERVAL блоков (= HALVING_YEARS лет)
        делится пополам. Округляется до делимости монеты (DECIMALS) и становится
        0, как только опускается ниже наименьшей единицы — это конец эмиссии.
        """
