"""
sha2.py — SHA-256 и SHA-512 «с нуля», без hashlib (по FIPS 180-4).

Реализация написана вручную в учебных целях: дополнение сообщения, расписание
сообщений (message schedule) и раунды сжатия с корректными константами.
Результат побитово совпадает со стандартом (проверено на векторах NIST и сверено
с hashlib в тестах).

Внимание: чистый Python в сотни раз медленнее hashlib, поэтому в самом блокчейне
(майнинг) используется hashlib. Этот модуль — корректная демонстрация алгоритма.
"""
...
def sha256_bytes(message) -> bytes:
    ...
    for base in range(0, len(message), 64):
        ...
        for i in range(64):
            ...
            ch = (e & f) ^ (~e & _MASK32 & g)
            t1 = (hh + S1 + ch + _K256[i] + w[i]) & _MASK32
...
def sha512_bytes(message) -> bytes:
    """SHA-512 сырыми байтами (64 байта)."""
    if isinstance(message, str):
        message = message.encode("utf-8")
    message = bytearray(message)

    bit_len = (len(message) * 8) & ((1 << 128) - 1)
    message.append(0x80)
    ...
    h = list(_H512)
    for base in range(0, len(message), 128):
        block = message[base:base + 128]
        ...
        for i in range(80):
            ...
            ch = (e & f) ^ (~e & _MASK64 & g)
            t1 = (hh + S1 + ch + _K512[i] + w[i]) & _MASK64
            S0 = _rotr64(a, 28) ^ _rotr64(a, 34) ^ _rotr64(a, 39)
            maj = (a & b) ^ (a & c) ^ (b & c)
            t2 = (S0 + maj) & _MASK64
            hh, g, f, e = g, f, e, (d + t1) & _MASK64
            d, c, b, a = c, b, a, (t1 + t2) & _MASK64
...
def sha512(message) -> str:
    ...
    return sha512_bytes(message).hex()
