...
Крипта — на НАШЕМ SHA-512 (`hashing`), как и весь проект, без сторонних
библиотек. Число итераций KDF масштабируется под скорость активного бэкенда:
на чистом Python (по умолчанию) чуть больше 1000 итераций дают ~0.3–0.5 c на
разблокировку; включив быстрый бэкенд (`BHYDRA_PURE_SHA=0`), можно задать
на порядки больше. Старые файлы хранят своё число итераций — совместимость.
"""
...
_VERSION = 1
_ITERATIONS = 2_000          # растяжение пароля (подобрано под чистый Python-SHA)
_SALT_LEN = 16
_NONCE_LEN = 16
...
def _derive_key(password: str, salt: bytes, iterations: int) -> bytes:
    """KDF: пароль + соль → 64-байтный ключ растяжением нашего SHA-512.
    ...
    block = hashing.sha512_bytes(salt + password.encode("utf-8"))
    for _ in range(iterations):
        block = hashing.sha512_bytes(block + salt)
    return block
...
def _keystream(key: bytes, nonce: bytes, length: int) -> bytes:
    ...
    while len(out) < length:
        out += hashing.sha512_bytes(key + nonce + counter.to_bytes(8, "big"))
        counter += 1
...
def _hmac_sha512(key: bytes, message: bytes) -> bytes:
    """HMAC на нашем SHA-512 (RFC 2104) — целостность и подлинность."""
    ...
    inner = hashing.sha512_bytes(i_pad + message)
    return hashing.sha512_bytes(o_pad + inner)
