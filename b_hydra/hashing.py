...
def sha512_hasher(data: "str | bytes" = b""):
    """Потоковый SHA-512 с `copy()` — тем же движком, что и разовый хеш.

    Нужен для МАЙНИНГА. В заголовке блока при переборе меняется только `nonce`
    в самом конце, а всё, что перед ним, — те же 400 с лишним байт на каждой
    попытке. Посчитав их ОДИН раз и копируя состояние (midstate, как в любом
    настоящем майнере), мы не пересжимаем четыре блока SHA-512 миллионы раз
    подряд. Замер: втрое быстрее.

    Копирование поддерживают ОБА движка — и `hashlib`, и наш `sha2.Sha512`, —
    поэтому ускорение получает и режим «всё своё», а не только быстрый путь.
    """
    raw = _to_bytes(data)
    return sha2.Sha512(raw) if _PURE else hashlib.sha512(raw)
