...
Мобильный кошелёк (REST API) — узел отдаёт JSON по HTTP, телефон подписывает
транзакции локально (приватный ключ не покидает устройство):

    python api.py --port 8000     # открой http://<IP>:8000/api/info с телефона
