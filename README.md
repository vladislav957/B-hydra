...
Структура проекта — ядро оформлено как Python-пакет b_hydra/:
    ...
    b_hydra/
      ...
      console.py             — терминал приложения (закрытый набор команд)
      updater.py, release.py — обновление с проверкой подписи релиза
...
майнингом и сетью (готовый `.exe` — в разделе Releases, или запуск из исходников):
    ...
    # вкладки: 💼 Кошелёк · ⛏ Майнинг · 🌐 Сеть · 🔍 Блоки · ⌨ Терминал
...
Возможности приложения:
  ...
  * Терминал — консоль команд внутри приложения (как окно отладки в Bitcoin
    Core): `getinfo`, `getblock`, `gettx`, `mine`, `send`, `peers`, `sync`.
    ⚠️ Это НЕ оболочка системы: набор команд закрытый, `eval`/`exec` там нет,
    приватный ключ ни одна команда не печатает, а `send` без `--yes` только
    показывает, что произойдёт. Мошенники давно просят «вставить команду для
    починки кошелька» — подтверждение существует именно поэтому.
