...
// ---------------------------------------------------------------- SHA-256
//
// ⚠️ Появился ради XMSS, а не ради консенсуса. Набор параметров P256 (режим по
// умолчанию в `pqcrypto.py`) строит цепочки Винтерница на SHA-256, и без него
// нативным был бы только «параноидальный» режим P512. В самой цепочке B-hydra
// SHA-256 не участвует нигде: там всюду SHA-512.
//
// Байты обязаны совпадать с `b_hydra/sha2.py` — сверяется в тестах, плюс
// вектор NIST прямо в self-test библиотеки.
class Sha256 {
public:
    Sha256() { reset(); }
    ...
    // Одноразовый хеш без выделения памяти — см. Sha512::one_shot.
    static void one_shot(const uint8_t *data, size_t len, uint8_t *out) {
        Sha256 h;
        size_t at = 0;
        while (len - at >= 64) {
            h.compress(data + at);
        ...
        uint8_t tail[128];
        size_t t = len - at;
        if (t) std::memcpy(tail, data + at, t);
        tail[t++] = 0x80;
        const size_t pad_to = (t <= 56) ? 56 : 120;
        while (t < pad_to) tail[t++] = 0x00;
        const uint64_t bits = static_cast<uint64_t>(len) * 8;
        for (int i = 7; i >= 0; --i) tail[t++] = (uint8_t)(bits >> (i * 8));
        for (size_t p = 0; p < t; p += 64) h.compress(tail + p);
        for (int i = 0; i < 8; ++i)
            for (int b = 3; b >= 0; --b)
                *out++ = (uint8_t)(h.state_[i] >> (b * 8));
    }
...
private:
    ...
    void compress(const uint8_t *block) {
        ...
        for (int i = 0; i < 16; ++i) {
            uint32_t value = 0;
            for (int b = 0; b < 4; ++b) value = (value << 8) | block[i * 4 + b];
            w[i] = value;
        }
        for (int i = 16; i < 64; ++i) {
...
inline Bytes sha256(const Bytes &data) {
    Bytes out(32);
    Sha256::one_shot(data.data(), data.size(), out.data());
    return out;
}
...
public:
    ...
    // Одноразовый хеш без единого выделения памяти.
    //
    // ⚠️ Нужен ради XMSS: генерация дерева высоты 16 — это ~70 млн хешей
    // коротких строк, и на таком объёме `Bytes buffer_` с его insert/erase
    // стоит дороже самого сжатия. Здесь полные блоки берутся прямо из входа,
    // а хвост дополняется в буфере на стеке.
    //
    // ⚠️ Это НЕ вторая реализация SHA-512: сжатие то же самое, `compress`.
    // Расходятся только подача байтов и дополнение, а они проверяются теми же
    // векторами, что и `digest()`.
    static void one_shot(const uint8_t *data, size_t len, uint8_t *out) {
        Sha512 h;
        size_t at = 0;
        while (len - at >= 128) {
        ...
        if (t) std::memcpy(tail, data + at, t);
        tail[t++] = 0x80;
        const size_t pad_to = (t <= 112) ? 112 : 240;
        while (t < pad_to) tail[t++] = 0x00;
        for (int i = 0; i < 8; ++i) tail[t++] = 0x00;   // старшие 64 бита длины
        const uint64_t bits = static_cast<uint64_t>(len) * 8;
        for (int i = 7; i >= 0; --i) tail[t++] = (uint8_t)(bits >> (i * 8));
        for (size_t p = 0; p < t; p += 128) h.compress(tail + p);
        for (int i = 0; i < 8; ++i)
            for (int b = 7; b >= 0; --b)
                *out++ = (uint8_t)(h.state_[i] >> (b * 8));
    }
