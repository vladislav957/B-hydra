...
/// Подпись ECDSA: приватный ключ + готовый хеш z → 64 байта r||s (low-s).
/// 1 — подписано, 0 — нет.
///
/// ⚠️ Хеш, как и у `bhydra_ec_verify`, остаётся в Python: там он уже посчитан,
/// и считать SHA-512 второй раз незачем.
///
/// ⚠️ Нонс k выводится по RFC 6979 из (priv, z) той же HMAC-SHA512-цепочкой,
/// что и в Python, поэтому подпись обязана совпадать с чистым Python БАЙТ В
/// БАЙТ. Это не украшение: подпись в проекте намеренно воспроизводима, и
/// разойдись две реализации — один и тот же перевод получал бы разные txid у
/// узлов с библиотекой и без неё. Совпадение проверяется тестом на живых
/// подписях, а не принимается на веру.
///
/// ⚠️ Приватный ключ живёт здесь ровно на время вызова и наружу не уходит —
/// возвращается только подпись.
int bhydra_ec_sign(const uint8_t* priv, const uint8_t* z, uint8_t* out) {
    if (priv == nullptr || z == nullptr || out == nullptr) {
        return 0;
    }
    const U256 secret = load(priv);
    const U256 n = bhydra::group_n();
    // Ключ обязан лежать в 1..n-1, иначе подпись бессмысленна.
    if (secret.is_zero() || U256::cmp(secret, n) >= 0) {
        return 0;
    }
    try {
        const Bytes signature = bhydra::sign_hash(secret, load(z));
        ...
        // Наружу летит только «не смогли»: подробности отказа подписи ничего
        // не дают вызывающему, а исключение через границу C сломало бы ctypes.
        return 0;
    }
    return 1;
}
