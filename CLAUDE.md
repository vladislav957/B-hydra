...
python -m pytest -q                           # 225 тестов — держать зелёными
...
`tests/` (20 файлов, 225 тестов, `pytest`). Полная карта слоёв — `ARCHITECTURE.md`,
...
- **Перед push — `python -m pytest -q` должно быть зелёным** (225/225).
