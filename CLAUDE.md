...
python -m pytest -q                           # 311 тестов — держать зелёными
...
`tests/` (22 файла, 311 тестов, `pytest`). Полная карта слоёв — `ARCHITECTURE.md`,
...
- **Перед push — `python -m pytest -q` должно быть зелёным** (311/311).
