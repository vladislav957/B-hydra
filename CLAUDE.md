...
python -m pytest -q                           # 315 тестов — держать зелёными
...
`tests/` (22 файла, 315 тестов, `pytest`). Полная карта слоёв — `ARCHITECTURE.md`,
...
- **Синхронизация ИНКРЕМЕНТАЛЬНАЯ и пачками** (`get_blocks`/`get_hashes`,
  ...
  ⚠️ Лимита на размер блока В БАЙТАХ в консенсусе НЕТ — только
  `MAX_BLOCK_TRANSACTIONS=5000` (~3,8 МБ при обычной транзакции ~795 байт).
  Транзакция с множеством входов/выходов может быть сколь угодно большой.
...
- **Перед push — `python -m pytest -q` должно быть зелёным** (315/315).
